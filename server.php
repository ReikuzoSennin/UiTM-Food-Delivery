...
// connect to database
$con = mysqli_connect('localhost', 'root', '', 'uitm_food_delivery');
...
function enterShop () {
	...
	if (mysqli_num_rows($result) == 1) { // user found
		...
		header('location: ../shops/gerai.php');	
	}
}
...
function uploadImage() {
	...
	if($file_size > 2097152){
	   $errors[]='File size larger than 2 MB';
	}
...
function addItem() {
	...
	if ($item) { // if shop name already exists
...
function addToCart() {
    if(isset($_SESSION["shopping_cart"])) {
        ...
        if(!in_array($_GET["itemID"], $item_array_ids)) {
